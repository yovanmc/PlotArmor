...
- **Date:** 2026-06-19
- **Status:** **Slices 1, 2 & 3a IMPLEMENTED** (2026-06-19) — Slice 1: Protagonist + 4 classes with composition abilities. Slice 2: per-class 5★ stars funded by global **Edits** from bosses (save v4). Slice 3a: cosmetic `(class × world)` skins earned by clearing world bosses (deterministic, no gacha) + equip on cards (save v5). All shipped on `main`. North-star locked; **Slice 3b (the 2/3/5 set bonus, §6b) is next**; the full collection gallery (§8) and the Protagonist Royalties track (§7) are deferred follow-ups; Slice 4 (affinity) deferred (§9). Only tuning magnitudes remain open (§12).
...
- **Slice 3b — The 2/3/5 set bonus (NEXT). [design LOCKED — §6b]** Per-world set bonus at uniform 2/3/5 fielded-variant thresholds; bonus type differs per world; folds into the `effective*` read-paths. Plus deferred: full collection gallery / party-selection visual pass (§8) and the Protagonist Royalties track (§7).
